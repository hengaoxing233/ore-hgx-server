...
pub async fn get_config(client: Arc<RpcClient>, ore: bool) -> Result<Config, &'static str> {
    match client
        .get_account_data(&if ore { ORE_CONFIG_ADDRESS } else { CONFIG_ADDRESS })
        .await {
        Ok(data) => { Ok(*Config::try_from_bytes(&data).expect("Failed to parse config account")) }
        Err(_) => {return Err("Failed to get config account")}
    }
}
